...
class Master(node: Node, data: Data) {
  ...
  def gradient(epochs: Int): Future[SparseVector] = {
    ...
    val result = (0 until epochs).foldLeft(init) {
      case (weightsEpoch, epoch) =>
        ...
        (0 until piece).foldLeft(weightsEpoch) { case (weightStep, step) =>
          //log.debug(s"step $step")

          weightStep.flatMap { weights =>
            val work = workers.zipWithIndex.map {
              case (worker, i) =>
                val sample = i * piece + step
                val req = GradientRequest(sample to (sample + 1), 0.001, 0, weights)
